...
class RealStockAPIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            try:
                ...
                # Get fresh fundamentals with ALWAYS estimated FCF
    ...
    def get_sector_multiplier_text(self, sector):
        ...
        return sector_multipliers.get(sector, sector_multipliers["default"])

    def get_accurate_fundamentals_with_estimated_fcf(self, symbol):
        """Get accurate fundamentals - ALWAYS use estimated FCF for Indian stocks"""
        try:
            # Ensure .NS suffix
            if not symbol.endswith(('.NS', '.BO')):
                symbol = f"{symbol}.NS"
            ...
            # Get Yahoo's FCF (for reference only - it's usually wrong for Indian stocks)
            yahoo_free_cash_flow = info.get('freeCashflow', 0)
            yahoo_fcf_per_share = round(
                yahoo_free_cash_flow / shares, 2) if shares else 0

            # ✅✅✅ CRITICAL FIX: ALWAYS use estimated FCF for Indian stocks (.NS, .BO)
            ...
            if symbol.endswith(('.NS', '.BO')):
                ...
                # For non-Indian stocks, use Yahoo FCF if it seems reasonable
                if yahoo_fcf_per_share < eps * 0.3 or yahoo_fcf_per_share > eps * 1.5:
                    final_fcf_per_share = estimated_fcf_per_share
                    fcf_source = "estimated"
            ...
            return {
                ...
                # FCF data - ALWAYS estimated for Indian stocks
